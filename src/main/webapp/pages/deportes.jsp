...
<%@ page import="deporte.dominio.Deporte"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<body>
	...
	<div class="p-10 sm:ml-64">
		<div class="text-black-900 text-[25px] font-semibold mt-10">Deportes</div>
		...
		<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
			<table
				...
				<thead
					...
					<tr>
						<th scope="col" class="px-6 py-3">Nombre del deporte</th>
        ...
        %>
					<tr
						...
						<th scope="row"
							...
							<%= deporte.getNombre() %></th>
						<td class="px-6 py-4"><%= deporte.getDescripcion() %></td>
						<td class="px-6 py-4"><span data-modal-target="editar-<%= deporte.getId() %>" data-modal-toggle="editar-<%= deporte.getId() %>"
    ...
    <path stroke="currentColor" stroke-linecap="round"
						...
						<td class="px-6 py-4"><span data-modal-target="eliminar-<%= deporte.getId() %>" data-modal-toggle="eliminar-<%= deporte.getId() %>"
    ...
    <path stroke="currentColor" stroke-linecap="round"
				...
				<div id="eliminar-<%= deporte.getId() %>" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="eliminar-<%= deporte.getId() %>">
            ...
            <form action="SvDeleteSport" method="POST">
                ...
                <button data-modal-hide="eliminar-<%= deporte.getId() %>" type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center me-2">
                    Si, estoy seguro
                </button>
                </form>
                <button data-modal-hide="eliminar-<%= deporte.getId() %>" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No, cancelar</button>
            </div>
        </div>
    </div>
</div>

<div id="editar-<%= deporte.getId() %>" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full p-8">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="p-4 md:p-5 relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="editar-<%= deporte.getId() %>">
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                    </svg>
                    <span class="sr-only">Close modal</span>
            ...
            <form class="p-4 md:p-5" action="SvUpdateSport" method="POST">
                ...
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
                        ...
                        placeholder="Escriba el nombre del evento" required="" value="<%= deporte.getNombre() %>">
                    </div>
                    <div class="col-span-2">
                        <label for="descripcion" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Descripcion</label>
                        <input type="text" name="descripcion-edit" id="descripcion-edit" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        placeholder="Escriba la Descripcion" required="" value="<%= deporte.getDescripcion() %>">
